...
const GUIDE_STEPS = ['填 PDF 路径', '提取预览', '开始翻译', '查看会话']
...
function Panel({ hide, openSession }: { hide: () => void; openSession?: (id: string) => void }): any {
  ...
  }, [path, pages, bilingual, appendix, remember])
  ...
  return e('div', { className: 'pdf2zh-shell', role: 'region', 'aria-label': 'PDF 英转中' },
    ...
    e('main', { className: 'pdf2zh-scroll' },
      e('div', { className: 'pdf2zh-content' },
        ...
        session !== null
          ? Section({ title: '翻译会话已创建', note: session.title, accent: 'success', children: [
            e('div', { className: 'pdf2zh-mut' },
              '模型正按 pdf2zh 技能逐节翻译，工作区 ',
              e('code', null, session.cwd),
              '。完成后会话会汇报输出文件路径与新增术语。',
            ),
            e('div', { className: 'pdf2zh-actions' },
...
export function apply(ctx: any): { dispose: () => void } | void {
  try {
    ...
    // Deep-link: open a session in the conversation column. The `sessions`
    // client service is provided by dsh-api-session-controller and may not be
    // registered before this plugin's apply runs, so resolve it lazily at
    // click time instead of capturing it up front.
    let openSession: ((id: string) => void) | undefined
    const resolveSessions = (): { open?: (id: string) => unknown } | undefined =>
      safe(() => ctx?.get?.('sessions')) as { open?: (id: string) => unknown } | undefined
