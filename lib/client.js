...
window.__ModuleLoader__.load({
	...
	factory: function(require) {
		...
		*
		* Panel: PDF path input (+ recent paths from localStorage), options
		* (pages / 中英对照 / 含附录), extract preview with stat chips, one-click
		* session translation with deep-link, glossary preview, and a status strip
		* with green/red health dots.
		*
		...
		*/
		...
		function Panel({ hide, openSession }) {
			...
			return e("div", {
			...
			}, e("span", { className: "pdf2zh-guide-num" }, String(i + 1)), step, i < GUIDE_STEPS.length - 1 ? e("span", { className: "pdf2zh-guide-arrow" }, "→") : null))), Section({
				title: "翻译论文",
				note: "服务器上的 PDF 绝对路径",
				children: [
					e("input", {
					...
					e("div", { className: "pdf2zh-options" }, e("label", { className: "pdf2zh-field" }, "页码", e("input", {
						style: {
							...INPUT_STYLE,
							width: 110
						},
					...
					e("div", { className: "pdf2zh-actions" }, e("button", {
						...
						disabled: !pathValid || busy,
						onClick: onTranslate
					}, translating ? "创建会话中…" : "开始翻译（新建会话）"), !pathValid ? e("span", { className: "pdf2zh-hint" }, "回车 = 提取预览") : null)
				]
			}), error !== "" ? e("div", { className: "pdf2zh-error" }, error) : null, extract !== null ? Section({
				title: "提取完成",
				accent: "success",
			...
			}) : null, session !== null ? Section({
				title: "翻译会话已创建",
				note: session.title,
				accent: "success",
				children: [e("div", { className: "pdf2zh-mut" }, "模型正按 pdf2zh 技能逐节翻译，工作区 ", e("code", null, session.cwd), "。完成后会话会汇报输出文件路径与新增术语。"), e("div", { className: "pdf2zh-actions" }, openSession !== void 0 ? e("button", {
					type: "button",
					className: "pdf2zh-btn pdf2zh-btn-primary",
					onClick: () => {
				...
				title: "术语表",
				note: "跨论文译名一致",
				children: [glossary === null ? e("div", { className: "pdf2zh-mut" }, "暂不可用") : e("div", null, glossaryLines.length > 0 ? e("div", { className: "pdf2zh-glossary-sample" }, glossaryLines.map((l) => e("div", {
					key: l,
					className: "pdf2zh-glossary-line"
				}, l)), glossary.terms > glossaryLines.length ? e("div", { className: "pdf2zh-mut" }, `… 共 ${glossary.terms} 条`) : null) : e("div", { className: "pdf2zh-mut" }, "（空）"), e("details", { className: "pdf2zh-preview" }, e("summary", null, `全文（${glossary.terms} 条）`), e("pre", null, glossary.text)))]
			...
			}, `v${health.version} · ${health.python}${health.skill.synced ? ` · ${health.skill.dir}` : ""}`)) : healthError !== "" ? e("span", { className: "pdf2zh-error" }, healthError) : e("span", { className: "pdf2zh-mut" }, "连接中…")));
		}
...
`;
		...
		function apply(ctx) {
			try {
				...
				let openSession;
				const resolveSessions = () => safe(() => ctx?.get?.("sessions"));
				openSession = (id) => {
					const open = resolveSessions()?.open;
					if (open !== void 0) {
						safe(() => open(id));
						controller.hide();
					}
