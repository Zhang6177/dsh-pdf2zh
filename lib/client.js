		...
		*/
		...
		function JobRow({ job, onDelete }) {
			...
			return e("div", {
				...
				value: job.progress,
				status: job.status
			}), e("span", { className: `pdf2zh-job-pct pdf2zh-job-pct-${job.status}` }, `${pct}%`)), job.status === "done" && (job.outputPaths?.length ?? 0) > 0 ? e("div", { className: "pdf2zh-job-meta" }, job.outputPaths.map((p) => e("span", {
				key: p,
		...
		function Panel({ hide }) {
			...
			const [savedOutputDir, setSavedOutputDir] = (0, react.useState)(null);
			...
			const refreshSettings = (0, react.useCallback)((fill) => {
				api.settings().then((s) => {
			...
			const onSaveSettings = (0, react.useCallback)(async () => {
				...
				try {
					...
					setNotice(saved.outputDir ? `设置已保存：译文 .zh.md 将保存到 ${saved.outputDir}` : "设置已保存：译文将保存在源 PDF 同目录");
					refreshJobs();
				} catch (err) {
					setError(err?.message ?? String(err));
				} finally {
			...
			return e("div", {
				...
				note: "翻译结果保存位置",
				children: [
					e("div", { className: "pdf2zh-field-row" }, e("span", { className: "pdf2zh-field-label" }, "保存路径"), e("input", {
						className: "pdf2zh-input",
						style: INPUT_STYLE,
						value: outputDir,
					...
					e("div", { className: "pdf2zh-mut" }, "指定后，翻译产出的中文 Markdown（.zh.md，含中英对照版）会统一保存到该目录（需为服务器上的绝对路径）；留空则保持与源 PDF 同目录。"),
					e("div", { className: "pdf2zh-actions" }, e("button", {
						type: "button",
						className: "pdf2zh-btn pdf2zh-btn-primary",
						disabled: !settingsDirty || savingSettings,
						onClick: onSaveSettings
					}, savingSettings ? "保存中…" : "保存设置"), savedOutputDir !== null ? e("span", { className: "pdf2zh-hint" }, savedOutputDir === "" ? "当前：源 PDF 同目录" : `当前：${savedOutputDir}`) : null)
				]
			}), Section({
				title: "术语表",
				note: "跨论文译名一致 · 可编辑",
