		...
		*/
		...
		function SettingsModal({ settings, onClose, onSaved }) {
			...
			const saveForm = (0, react.useCallback)(async () => {
				...
				try {
					...
					setNotice(next.outputDir ? `已保存：译文保存到 ${next.outputDir}（并发 ${next.concurrency ?? 8}，超时 ${next.timeoutMinutes} 分钟）` : `已保存：译文保存在源 PDF 同目录（并发 ${next.concurrency ?? 8}，超时 ${next.timeoutMinutes} 分钟）`);
				} catch (err) {
					setError(err?.message ?? String(err));
				} finally {
					setSavingForm(false);
			...
			}), e("span", { className: "pdf2zh-preset-unit" }, "并发"))), e("div", { className: "pdf2zh-field-hint" }, "1–16。同时向模型服务发出的翻译请求数；本地 vLLM 建议 8。数值过高会挤占同一 GPU 上其它会话的吞吐。")), e("div", { className: "pdf2zh-field-card" }, e("div", { className: "pdf2zh-field-label" }, e("span", { className: "pdf2zh-field-icon" }, svg(ICONS.clock, 14)), "任务超时"), e("div", { className: "pdf2zh-preset-row" }, TIMEOUT_PRESETS.map((p) => e("button", {
				key: p.v,
				type: "button",
		...
		function JobRow({ job, onDelete, onRetry }) {
			...
			return e("div", {
				...
				value: job.progress,
				status: job.status
			}), e("span", { className: `pdf2zh-job-pct pdf2zh-job-pct-${job.status}` }, `${pct}%`), job.status === "running" && job.phase ? e("span", { className: "pdf2zh-job-phase" }, job.phase) : null), job.provider !== "" || job.modelNote !== void 0 ? e("div", { className: "pdf2zh-job-meta" }, job.provider !== "" ? e("span", { className: "pdf2zh-job-api" }, svg(ICONS.plug, 11), ` ${job.provider}/${job.model}`) : null, job.modelNote ? e("span", { className: "pdf2zh-job-warn" }, job.modelNote) : null, job.status === "done" && job.stats?.pages ? e("span", { className: "pdf2zh-job-stats" }, `${job.stats.pages} 页 · ${job.stats.to_translate ?? "?"} 段 · 公式 ${job.stats.math_items ?? 0} · 图 ${job.stats.images ?? 0}`) : null) : null, job.status === "done" && (job.outputPaths?.length ?? 0) > 0 ? e("div", { className: "pdf2zh-job-meta" }, job.outputPaths.map((p) => e("a", {
				key: p,
