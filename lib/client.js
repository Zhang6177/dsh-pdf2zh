	...
	factory: function(require) {
		...
		*/
		...
		const api = {
			...
			modelsDiscover: (body) => call(`${API_PREFIX}/models/discover`, "POST", body),
			modelsAdd: (body) => call(`${API_PREFIX}/models/add`, "POST", body),
			modelsRemove: (body) => call(`${API_PREFIX}/models/remove`, "POST", body),
			jobs: () => call(`${API_PREFIX}/jobs`, "GET"),
			jobDelete: (id) => call(`${API_PREFIX}/jobs/delete`, "POST", { id }),
			jobRetry: (id) => call(`${API_PREFIX}/jobs/retry`, "POST", { id }),
		...
		function readRecentList(key, max) {
			try {
		...
		function pushRecentDir(dir) {
			...
			try {
				localStorage.setItem(RECENT_DIR_KEY, JSON.stringify(next));
			} catch {}
			return next;
		}
		...
		function Section(props) {
			...
			return e("div", { className: `pdf2zh-section${accent ? ` pdf2zh-section-${accent}` : ""}` }, e("div", { className: "pdf2zh-section-head" }, icon ? e("span", { className: "pdf2zh-section-icon" }, svg(ICONS[icon], 15)) : null, e("span", { className: "pdf2zh-section-title" }, title), note ? e("span", { className: "pdf2zh-section-note" }, note) : null), e("div", { className: "pdf2zh-section-body" }, children));
		}
		...
		function ProgressBar({ value, status }) {
			...
			return e("div", { className: `pdf2zh-bar${status === "running" ? " pdf2zh-bar-anim" : ""}` }, e("div", {
				className: `pdf2zh-bar-fill pdf2zh-bar-${status}`,
				style: { width: `${pct}%` }
			}));
		}
		/** Green/red feedback strips with a leading glyph. */
		function Strip({ kind, text }) {
			return e("div", { className: `pdf2zh-strip pdf2zh-strip-${kind}` }, e("span", { className: "pdf2zh-strip-icon" }, svg(kind === "notice" ? ICONS.check : ICONS.warn, 15)), text);
		}
		const API_PROTOCOLS = [
			{
				v: "openai-completions",
				label: "OpenAI 兼容（vLLM / 网关）"
			},
			{
		...
		function fmtCtx(n) {
			...
			return n >= 1048576 ? `${Math.round(n / 1048576 * 10) / 10}M` : `${Math.round(n / 1024)}k`;
		}
		function AddAPIForm({ taken, onAdded, onCancel }) {
			const [displayName, setDisplayName] = (0, react.useState)("");
			const [providerManual, setProviderManual] = (0, react.useState)("");
			const [proto, setProto] = (0, react.useState)("openai-completions");
			...
			const [wantDefault, setWantDefault] = (0, react.useState)(true);
			...
			const chosen = discovered.filter((m) => selected[m.id]).map((m) => ({
				id: m.id,
			...
			const discover = (0, react.useCallback)(async () => {
				setError("");
				setHint("");
				if (!/^https?:\/\//i.test(baseURL.trim())) {
					setError("请先填写 http(s) 服务地址");
					return;
				}
				setDiscovering(true);
				try {
					const r = await api.modelsDiscover({
			...
			const submit = (0, react.useCallback)(async () => {
				...
				if (!/^[a-z][a-z0-9_-]{1,40}$/.test(providerId)) {
					setError("API 标识需为小写字母开头的 2–41 位字母/数字/-/_（可编辑上方显示名或手动指定）");
					return;
				}
				if (taken.includes(providerId)) {
					setError(`标识 "${providerId}" 已存在，换一个或先删除旧的`);
					return;
				}
				if (!/^https?:\/\//i.test(baseURL.trim())) {
					setError("服务地址需为 http(s) URL");
					return;
				}
				if (allModels.length === 0) {
					setError("请先「获取模型」或手动填写至少一个模型 id");
					return;
				}
				setBusy(true);
				try {
					onAdded(await api.modelsAdd({
						provider: providerId,
			...
			return e("div", { className: "pdf2zh-addform" }, e("div", { className: "pdf2zh-addform-grid" }, e("div", { className: "pdf2zh-field-row" }, e("span", { className: "pdf2zh-field-label" }, "显示名"), e("input", {
				className: "pdf2zh-input",
				style: INPUT_STYLE,
				value: displayName,
				placeholder: "例如 DeepSeek 官方 / 本地 vLLM",
				onChange: (ev) => setDisplayName(ev.target.value),
				spellCheck: false
			})), e("div", { className: "pdf2zh-field-row" }, e("span", { className: "pdf2zh-field-label" }, "API 标识", e("span", { className: "pdf2zh-label-dim" }, "（小写 id）")), e("input", {
				className: "pdf2zh-input pdf2zh-input-mono",
				style: INPUT_STYLE,
				value: providerManual,
				placeholder: `自动：${slugify(displayName) || "my-api"}`,
				onChange: (ev) => setProviderManual(ev.target.value),
				spellCheck: false
			}))), e("div", { className: "pdf2zh-field-row" }, e("span", { className: "pdf2zh-field-label" }, "协议"), e("div", { className: "pdf2zh-preset-row" }, API_PROTOCOLS.map((p) => e("button", {
				key: p.v,
				type: "button",
				className: `pdf2zh-preset${proto === p.v ? " pdf2zh-preset-on" : ""}`,
				onClick: () => setProto(p.v)
			}, p.label)))), e("div", { className: "pdf2zh-field-row" }, e("span", { className: "pdf2zh-field-label" }, e("span", { className: "pdf2zh-field-icon" }, svg(ICONS.plug, 14)), "服务地址"), e("input", {
				className: "pdf2zh-input pdf2zh-input-mono",
				style: INPUT_STYLE,
				value: baseURL,
				placeholder: "http://127.0.0.1:8000/v1 或 https://api.deepseek.com/v1",
				onChange: (ev) => setBaseURL(ev.target.value),
				spellCheck: false
			})), e("div", { className: "pdf2zh-field-row" }, e("span", { className: "pdf2zh-field-label" }, "API Key", e("span", { className: "pdf2zh-label-dim" }, "（无鉴权端点可留空）")), e("div", { className: "pdf2zh-key-wrap" }, e("input", {
				className: "pdf2zh-input pdf2zh-input-mono",
				style: INPUT_STYLE,
			...
			}, showKey ? "隐藏" : "显示")), e("div", { className: "pdf2zh-field-hint" }, "密钥存入 dsh 凭据库（~/.dsh/.credentials.yaml，与「设置 → 模型」同一位置），界面不回显明文。")), e("div", { className: "pdf2zh-field-row" }, e("div", { className: "pdf2zh-model-tools" }, e("span", { className: "pdf2zh-field-label" }, e("span", { className: "pdf2zh-field-icon" }, svg(ICONS.doc, 14)), "模型"), e("span", { style: { flex: 1 } }), e("button", {
				type: "button",
				className: "pdf2zh-btn pdf2zh-btn-mini",
			...
			}, discovering ? "探测中…" : discovered.length > 0 ? "重新获取" : "获取模型（兼测试连接）")), hint !== "" ? e("div", { className: "pdf2zh-mut" }, hint) : null, discovered.length > 0 ? e("div", { className: "pdf2zh-model-picks" }, discovered.map((m) => e("button", {
				key: m.id,
				type: "button",
			...
			}, m.id, typeof m.contextWindow === "number" ? e("span", { className: "pdf2zh-pick-ctx" }, fmtCtx(m.contextWindow)) : null))) : null, e("textarea", {
				className: "pdf2zh-add-manual",
				value: manual,
				placeholder: "手动补充模型 id（每行一个，选填）",
				spellCheck: false,
				onChange: (ev) => setManual(ev.target.value)
			}), allModels.length > 0 ? e("div", { className: "pdf2zh-field-hint" }, `将注册 ${allModels.length} 个模型：${allModels.slice(0, 6).map((m) => m.id).join("、")}${allModels.length > 6 ? " …" : ""}`) : null), error !== "" ? e(Strip, {
				kind: "error",
				text: error
			}) : null, e("div", { className: "pdf2zh-add-actions" }, e("label", { className: "pdf2zh-check" }, e("input", {
				type: "checkbox",
				checked: wantDefault,
				onChange: (ev) => setWantDefault(ev.target.checked)
			}), "添加后设为默认"), e("span", { style: { flex: 1 } }), e("button", {
				type: "button",
		...
		function SettingsModal({ settings, onClose, onSaved }) {
			const [tab, setTab] = (0, react.useState)("api");
			...
			const [concurrency, setConcurrency] = (0, react.useState)(String(settings?.concurrency ?? 8));
			const [fontShrink, setFontShrink] = (0, react.useState)(String(settings?.fontShrink ?? 1.5));
			const [recentDirs, setRecentDirs] = (0, react.useState)(() => readRecentList(RECENT_DIR_KEY, RECENT_DIR_MAX));
			...
			const reloadModels = (0, react.useCallback)(() => {
				api.models().then((r) => {
					setModels(r);
					setModelsError("");
				}).catch((err) => setModelsError(err?.message ?? String(err)));
			}, []);
			(0, react.useEffect)(() => {
				reloadModels();
			}, [reloadModels]);
			...
			const saveModel = (0, react.useCallback)(async (sel) => {
				...
				try {
					...
					setNotice(sel.provider === "" ? "已切回自动选择（优先本地部署 API）" : `默认翻译 API 已设为 ${sel.provider} / ${sel.model}`);
				} catch (err) {
					setError(err?.message ?? String(err));
				} finally {
					setSavingModel("");
			...
			const saveForm = (0, react.useCallback)(async () => {
				...
				try {
					const next = await api.settingsSave({
						...
						concurrency: Number(concurrency) || void 0,
						fontShrink: Number(fontShrink)
					});
					onSaved(next);
					if (next.outputDir !== "") setRecentDirs(pushRecentDir(next.outputDir));
					setNotice(next.outputDir ? `已保存：译文保存到 ${next.outputDir}（并发 ${next.concurrency ?? 8}，字号收缩 ${next.fontShrink ?? 1.5}pt）` : `已保存：译文保存在源 PDF 同目录（并发 ${next.concurrency ?? 8}，字号收缩 ${next.fontShrink ?? 1.5}pt）`);
				} catch (err) {
					setError(err?.message ?? String(err));
				} finally {
					setSavingForm(false);
			...
			const onProviderAdded = (0, react.useCallback)(async (r, wantDefault) => {
				setAddOpen(false);
				reloadModels();
			...
			const onProviderRemove = (0, react.useCallback)(async (provider) => {
				setDeleting("");
				setError("");
				try {
					...
					if (saved.provider === provider) {
						const next = await api.settingsSave({ model: {
							provider: "",
							model: ""
						} }).catch(() => null);
			...
			}, opts.icon, e("span", { className: "pdf2zh-api-text" }, e("span", { className: "pdf2zh-api-name" }, opts.name), e("span", { className: "pdf2zh-api-sub" }, opts.sub)), opts.tags ?? null, e("span", { className: `pdf2zh-api-check${opts.active ? " pdf2zh-api-check-on" : ""}` }, opts.active ? svg(ICONS.check, 12) : null));
			const modelRow = (providerId, providerName, m, isAutoLocal) => {
				...
				return apiRow({
					...
					tags: e("span", { className: "pdf2zh-api-tags" }, isAutoLocal ? e("span", { className: "pdf2zh-api-tag pdf2zh-api-tag-local" }, "本地") : null, savingModel === key ? e("span", { className: "pdf2zh-api-tag" }, "保存中…") : null)
				});
			};
			return e("div", {
			...
			}, e("div", { className: "pdf2zh-modal-head" }, e("span", { className: "pdf2zh-modal-icon" }, svg(ICONS.gear, 17)), e("div", { className: "pdf2zh-modal-titlewrap" }, e("div", { className: "pdf2zh-modal-title" }, "设置"), e("div", { className: "pdf2zh-modal-sub" }, "pdf2zh · 翻译模型 API 与输出")), e("button", {
				type: "button",
				className: "pdf2zh-modal-x",
				onClick: onClose,
				title: "关闭（Esc）"
			}, "×")), e("div", { className: "pdf2zh-tabs" }, e("button", {
				type: "button",
				className: `pdf2zh-tab-btn${tab === "api" ? " pdf2zh-tab-on" : ""}`,
				onClick: () => setTab("api")
			}, svg(ICONS.plug, 14), "模型 API"), e("button", {
				type: "button",
				className: `pdf2zh-tab-btn${tab === "output" ? " pdf2zh-tab-on" : ""}`,
				onClick: () => setTab("output")
			}, svg(ICONS.folder, 14), "输出与性能")), e("div", { className: "pdf2zh-modal-body" }, tab === "api" ? e(react.default.Fragment, null, e("div", { className: "pdf2zh-modal-desc" }, svg(ICONS.info, 14), e("span", null, "与 dsh 共用同一份 API 注册表，点选即设为翻译默认；也可点下方按钮手动添加新的 API（URL / Key / 模型），写入 dsh 模型配置后全局可用。")), models !== null && models.canManage !== false && !addOpen ? e("button", {
				type: "button",
				className: "pdf2zh-addbtn",
				onClick: () => setAddOpen(true)
			}, svg(ICONS.plus, 13), "手动添加 API（填写服务地址 / Key / 模型）") : null, addOpen ? e(AddAPIForm, {
				taken: (models?.providers ?? []).map((p) => p.id),
				onAdded: (r, wantDefault) => {
					onProviderAdded(r, wantDefault);
				},
				...
				sub: models?.auto ? `当前解析为 ${models.auto.provider} / ${models.auto.model}` : "每次开始翻译时按模型目录实时解析",
			...
			}), models === null && modelsError === "" ? e("div", { className: "pdf2zh-mut" }, "正在读取模型目录…") : null, modelsError !== "" ? e(Strip, {
				kind: "error",
				text: modelsError
			}) : null, models !== null ? e("div", { className: "pdf2zh-api-groups" }, models.providers.map((g) => e("div", {
				key: g.id,
				className: `pdf2zh-api-card${g.routable ? "" : " pdf2zh-api-card-off"}`
			}, e("div", {
				className: "pdf2zh-api-card-head",
				title: g.base || void 0
			}, e("span", { className: "pdf2zh-api-avatar" }, (g.name || g.id).charAt(0).toUpperCase()), e("span", { className: "pdf2zh-api-card-name" }, e("span", null, g.name !== g.id ? g.name : g.id), g.name !== g.id ? e("code", { className: "pdf2zh-api-card-id" }, g.id) : null), e("span", { className: "pdf2zh-api-card-tags" }, g.userAdded === true ? e("span", { className: "pdf2zh-api-tag pdf2zh-api-tag-self" }, "自添加") : null, g.routable ? e("span", { className: "pdf2zh-api-tag pdf2zh-api-tag-ok" }, "可用") : e("span", { className: "pdf2zh-api-tag pdf2zh-api-tag-warn" }, "暂不可路由"), g.userAdded === true && models.canManage !== false ? deleting === g.id ? e(react.default.Fragment, null, e("button", {
				type: "button",
				className: "pdf2zh-del-btn pdf2zh-del-btn-confirm",
				onClick: () => {
					onProviderRemove(g.id);
				}
			}, "确认删除"), e("button", {
				type: "button",
			...
			}, "删除") : null)), e("div", { className: "pdf2zh-api-card-body" }, g.models.map((m) => modelRow(g.id, g.name, m, models.auto?.provider === g.id && models.auto?.model === m.id))))), (models.failures ?? []).length > 0 ? e("div", { className: "pdf2zh-modal-warnline" }, svg(ICONS.warn, 13), `部分 provider 读取失败：${models.failures.map((f) => f.id).join("、")}`) : null) : null) : e(react.default.Fragment, null, e("div", { className: "pdf2zh-modal-desc" }, svg(ICONS.info, 14), e("span", null, "翻译产物的统一落盘位置与管线参数；对之后新发起的翻译生效。")), e("div", { className: "pdf2zh-field-card" }, e("div", { className: "pdf2zh-field-label" }, e("span", { className: "pdf2zh-field-icon" }, svg(ICONS.folder, 14)), "保存路径"), e("input", {
				className: "pdf2zh-input pdf2zh-input-mono",
				style: INPUT_STYLE,
				value: outputDir,
				...
				spellCheck: false
			}), recentDirs.length > 0 ? e("div", { className: "pdf2zh-recent" }, e("span", { className: "pdf2zh-recent-label" }, "常用"), recentDirs.map((d) => e("button", {
				key: d,
				type: "button",
				className: "pdf2zh-recent-chip",
				title: d,
				onClick: () => setOutputDir(d)
			}, d.split("/").pop()))) : null, e("div", { className: "pdf2zh-field-hint" }, "需为服务器上的绝对路径（保存时自动创建）。每篇产出 <原名>.zh.pdf（排版保真）与 <原名>.zh.md。")), e("div", { className: "pdf2zh-field-card" }, e("div", { className: "pdf2zh-field-label" }, e("span", { className: "pdf2zh-field-icon" }, svg(ICONS.translate, 14)), "翻译并发"), e("div", { className: "pdf2zh-preset-row" }, CONCURRENCY_PRESETS.map((c) => e("button", {
				key: c,
				type: "button",
				className: `pdf2zh-preset${String(c) === concurrency ? " pdf2zh-preset-on" : ""}`,
				onClick: () => setConcurrency(String(c))
			}, `${c} 路`)), e("span", { className: "pdf2zh-preset-custom" }, e("input", {
				...
				min: 1,
				max: 16,
				onChange: (ev) => setConcurrency(ev.target.value)
			}), e("span", { className: "pdf2zh-preset-unit" }, "并发"))), e("div", { className: "pdf2zh-field-hint" }, "1–16。同时向模型服务发出的翻译请求数；本地 vLLM 建议 8。数值过高会挤占同一 GPU 上其它会话的吞吐。")), e("div", { className: "pdf2zh-field-card" }, e("div", { className: "pdf2zh-field-label" }, e("span", { className: "pdf2zh-field-icon" }, svg(ICONS.doc, 14)), "正文字号收缩"), e("div", { className: "pdf2zh-preset-row" }, FONT_SHRINK_PRESETS.map((f) => e("button", {
				key: f,
				type: "button",
				className: `pdf2zh-preset${String(f) === fontShrink ? " pdf2zh-preset-on" : ""}`,
				onClick: () => setFontShrink(String(f))
			}, f === 0 ? "原大小" : `−${f}pt`)), e("span", { className: "pdf2zh-preset-custom" }, e("input", {
				className: "pdf2zh-input pdf2zh-preset-input",
				style: {
					...INPUT_STYLE,
					padding: "6px 10px",
				...
				max: 3,
				step: .5,
				onChange: (ev) => setFontShrink(ev.target.value)
			}), e("span", { className: "pdf2zh-preset-unit" }, "pt"))), e("div", { className: "pdf2zh-field-hint" }, "中文字面视觉偏大：排版时正文统一收缩该磅数（0–3，默认 1pt）；标题按原层级同向收缩。")), e("div", { className: "pdf2zh-field-card" }, e("div", { className: "pdf2zh-field-label" }, e("span", { className: "pdf2zh-field-icon" }, svg(ICONS.clock, 14)), "任务超时"), e("div", { className: "pdf2zh-preset-row" }, TIMEOUT_PRESETS.map((p) => e("button", {
				key: p.v,
				type: "button",
				className: `pdf2zh-preset${String(p.v) === timeoutMinutes ? " pdf2zh-preset-on" : ""}`,
				onClick: () => setTimeoutMinutes(String(p.v))
			}, p.label)), e("span", { className: "pdf2zh-preset-custom" }, e("input", {
				className: "pdf2zh-input pdf2zh-preset-input",
				style: {
					...INPUT_STYLE,
					padding: "6px 10px",
			...
			}), e("span", { className: "pdf2zh-preset-unit" }, "分钟"))), e("div", { className: "pdf2zh-field-hint" }, "10–1440 分钟。超时后插件会终止翻译进程并把任务记为失败。"))), error !== "" ? e(Strip, {
				kind: "error",
				text: error
			}) : null, notice !== "" ? e(Strip, {
				kind: "notice",
				text: notice
			}) : null), e("div", { className: "pdf2zh-modal-foot" }, e("span", { className: "pdf2zh-foot-hint" }, tab === "api" ? "点选后立即保存为默认，无需其它操作" : "修改后点右侧保存"), tab === "output" ? e("button", {
				type: "button",
				className: "pdf2zh-btn pdf2zh-btn-primary",
				disabled: savingForm,
				onClick: saveForm
			}, savingForm ? "保存中…" : "保存") : null, e("button", {
				type: "button",
				className: "pdf2zh-btn",
				onClick: onClose
			}, "完成"))));
		}
		function JobRow({ job, onDelete, onRetry }) {
			...
			return e("div", {
				...
				key: job.id
			}, e("div", { className: "pdf2zh-job-head" }, e("span", { className: `pdf2zh-job-statusdot pdf2zh-job-statusdot-${job.status}` }), e("span", {
				className: "pdf2zh-job-name",
				title: job.pdfPath
			}, job.pdfName), e("span", { className: `pdf2zh-badge pdf2zh-badge-${job.status}` }, STATUS_LABEL[job.status]), job.status === "failed" ? e("button", {
				type: "button",
				...
				value: job.progress,
				status: job.status
			}), e("span", { className: `pdf2zh-job-pct pdf2zh-job-pct-${job.status}` }, `${pct}%`), job.status === "running" && job.phase ? e("span", { className: "pdf2zh-job-phase" }, job.phase) : null), job.provider !== "" || job.modelNote !== void 0 ? e("div", { className: "pdf2zh-job-meta" }, job.provider !== "" ? e("span", { className: "pdf2zh-job-api" }, svg(ICONS.plug, 11), ` ${job.provider}/${job.model}`) : null, job.modelNote ? e("span", { className: "pdf2zh-job-warn" }, job.modelNote) : null, job.status === "done" && job.stats?.pages ? e("span", { className: "pdf2zh-job-stats" }, `${job.stats.pages} 页 · ${job.stats.to_translate ?? "?"} 段 · 公式 ${job.stats.math_items ?? 0} · 图 ${job.stats.images ?? 0}` + (job.stats.figure_blocks ? ` · 图内文字保护 ${job.stats.figure_blocks}` : "")) : null) : null, job.status === "done" && (job.outputPaths?.length ?? 0) > 0 ? e("div", { className: "pdf2zh-job-meta" }, job.outputPaths.map((p) => e("a", {
				key: p,
				...
				target: "_blank",
				rel: "noreferrer"
			}, `↓ ${p.split("/").pop()}`))) : null, job.status === "done" && (job.outputPaths?.length ?? 0) === 0 ? e("div", { className: "pdf2zh-job-meta" }, job.note ?? "翻译已完成（输出文件不在记录中，请检查保存目录）。") : null, job.note && job.status !== "done" ? e("div", { className: "pdf2zh-job-meta" }, e("span", { className: "pdf2zh-job-warn" }, job.note)) : null, job.status === "failed" && job.error ? e("div", { className: "pdf2zh-job-meta" }, e("span", { className: "pdf2zh-job-error" }, job.error), authDown ? e("span", { className: "pdf2zh-job-hintline" }, "API 鉴权失败：在「⚙ 设置 → 模型 API」检查该 API 的 Key（可删除后重新添加），然后点「重试」。") : null, connDown && !authDown ? e("span", { className: "pdf2zh-job-hintline" }, "模型服务不在线：先在服务器启动对应服务（vLLM 等），或在「⚙ 设置 → 模型 API」换用其它可用 API，然后点「重试」。") : null) : null);
		}
		function StatCard({ status, value, label }) {
			return e("div", { className: `pdf2zh-statcard pdf2zh-statcard-${status}` }, e("div", { className: "pdf2zh-statcard-value" }, String(value)), e("div", { className: "pdf2zh-statcard-label" }, label));
		}
		function Board({ jobs, summary, onDelete, onClear, onRetry }) {
			...
			return Section({
				...
				icon: "board",
				children: [
					e("div", { className: "pdf2zh-statcards" }, e(StatCard, {
						status: "running",
					...
					total > 0 ? e("div", { className: "pdf2zh-overall" }, e("span", { className: "pdf2zh-overall-label" }, "总体进度"), e(ProgressBar, {
						value: overall,
						status: overallStatus
					}), e("span", { className: "pdf2zh-overall-pct" }, `${Math.round(overall * 100)}%`)) : null,
					total > 0 && summary.done + summary.failed > 0 ? e("div", { className: "pdf2zh-board-tools" }, e("button", {
						type: "button",
						className: "pdf2zh-btn pdf2zh-btn-mini",
						onClick: onClear
					}, "清空已完成")) : null,
					total === 0 ? e("div", { className: "pdf2zh-empty" }, e("span", { className: "pdf2zh-empty-icon" }, svg(ICONS.board, 22)), e("span", null, "暂无翻译任务 — 填好路径点「开始翻译」后，进度会在这里实时更新")) : e("div", { className: "pdf2zh-jobs" }, jobs.map((job) => e(JobRow, {
						key: job.id,
						job,
						onDelete,
						onRetry
		...
		function Panel({ hide }) {
			...
			const [recent, setRecent] = (0, react.useState)(() => readRecentList(RECENT_KEY, RECENT_MAX));
			...
			const onJobRetry = (0, react.useCallback)((id) => {
				setError("");
				setNotice("");
				api.jobRetry(id).then((r) => {
			...
			const modelLabel = settings === null ? "" : settings.model.provider !== "" ? ` · ${settings.model.provider}/${settings.model.model}` : " · API 自动（本地优先）";
			return e("div", {
				className: "pdf2zh-shell",
				role: "region",
				"aria-label": "PDF 英转中"
			}, e("header", { className: "pdf2zh-top" }, e("div", { className: "pdf2zh-top-inner" }, e("div", { className: "pdf2zh-heading" }, e("span", { className: "pdf2zh-logo" }, svg(ICONS.translate, 19)), e("div", { className: "pdf2zh-heading-text" }, e("div", { className: "pdf2zh-title" }, "PDF 英转中"), e("span", { className: "pdf2zh-tabtitle" }, "pdf2zh · 轻量化学术论文 PDF 英转中"))), e("div", { className: "pdf2zh-top-actions" }, e("button", {
				type: "button",
				...
				children: [
					...
					}), e("span", { className: "pdf2zh-drop-icon" }, svg(ICONS.upload, 19)), e("span", null, uploading ? `正在上传 ${uploadName}…` : "拖拽 PDF 到此处，或点击选择文件上传到服务器")),
					e("input", {
						className: "pdf2zh-input pdf2zh-input-mono",
						style: INPUT_STYLE,
						value: path,
					...
					e("div", { className: "pdf2zh-actions" }, e("button", {
						...
						disabled: !pathValid || busy,
						onClick: onTranslate
					}, translating ? "创建任务中…" : "开始翻译"), !pathValid ? e("span", { className: "pdf2zh-hint" }, "回车 = 提取预览") : null)
				]
			}), notice !== "" ? e(Strip, {
				kind: "notice",
				text: notice
			...
			}, `v${health.version} · ${health.python}${modelLabel}`)) : healthError !== "" ? e("span", { className: "pdf2zh-strip pdf2zh-strip-error" }, healthError) : e("span", { className: "pdf2zh-mut" }, "连接中…")), modalOpen ? e(SettingsModal, {
				settings,
