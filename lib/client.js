	...
	factory: function(require) {
		...
		*/
		...
		function SettingsModal({ settings, onClose, onSaved }) {
			...
			}, svg(ICONS.folder, 14), "输出与超时")), e("div", { className: "pdf2zh-modal-body" }, tab === "api" ? e(react.default.Fragment, null, e("div", { className: "pdf2zh-modal-desc" }, svg(ICONS.info, 14), e("span", null, "与 dsh 共用同一份 API 注册表：在 dsh「设置 → 模型」添加 DeepSeek / Qwen / 本地部署后，这里自动同步。点选即设为翻译默认。")), apiRow({
				key: "auto",
			...
			}, e("div", { className: "pdf2zh-api-card-head" }, e("span", { className: "pdf2zh-api-avatar" }, (g.name || g.id).charAt(0).toUpperCase()), e("span", { className: "pdf2zh-api-card-name" }, e("span", null, g.name !== g.id ? g.name : g.id), g.name !== g.id ? e("code", { className: "pdf2zh-api-card-id" }, g.id) : null), e("span", { className: "pdf2zh-api-card-tags" }, g.routable ? e("span", { className: "pdf2zh-api-tag pdf2zh-api-tag-ok" }, "可用") : e("span", { className: "pdf2zh-api-tag pdf2zh-api-tag-warn" }, "暂不可路由"))), e("div", { className: "pdf2zh-api-card-body" }, g.models.map((m) => modelRow(g.id, g.name, m, models.auto?.provider === g.id && models.auto?.model === m.id))))), (models.failures ?? []).length > 0 ? e("div", { className: "pdf2zh-modal-warnline" }, svg(ICONS.warn, 13), `部分 provider 读取失败：${models.failures.map((f) => f.id).join("、")}`) : null) : null) : e(react.default.Fragment, null, e("div", { className: "pdf2zh-modal-desc" }, svg(ICONS.info, 14), e("span", null, "译文（.zh.md / 中英对照 .en-zh.md）的统一落盘位置与任务超时；对之后新发起的翻译生效。")), e("div", { className: "pdf2zh-field-card" }, e("div", { className: "pdf2zh-field-label" }, e("span", { className: "pdf2zh-field-icon" }, svg(ICONS.folder, 14)), "保存路径"), e("input", {
