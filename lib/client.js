		...
		*/
		...
		function SettingsModal({ settings, onClose, onSaved }) {
			...
			const saveForm = (0, react.useCallback)(async () => {
				...
				try {
					...
					setNotice(next.outputDir ? `已保存：译文保存到 ${next.outputDir}（超时 ${next.timeoutMinutes} 分钟）` : `已保存：译文保存在源 PDF 同目录（超时 ${next.timeoutMinutes} 分钟）`);
				} catch (err) {
					setError(err?.message ?? String(err));
				} finally {
					setSavingForm(false);
			...
			}, svg(ICONS.folder, 14), "输出与超时")), e("div", { className: "pdf2zh-modal-body" }, tab === "api" ? e(react.default.Fragment, null, e("div", { className: "pdf2zh-modal-desc" }, svg(ICONS.info, 14), e("span", null, "与 dsh 共用同一份 API 注册表，点选即设为翻译默认；也可点下方按钮手动添加新的 API（URL / Key / 模型），写入 dsh 模型配置后全局可用。")), models !== null && models.canManage !== false && !addOpen ? e("button", {
				type: "button",
			...
			}, "删除") : null)), e("div", { className: "pdf2zh-api-card-body" }, g.models.map((m) => modelRow(g.id, g.name, m, models.auto?.provider === g.id && models.auto?.model === m.id))))), (models.failures ?? []).length > 0 ? e("div", { className: "pdf2zh-modal-warnline" }, svg(ICONS.warn, 13), `部分 provider 读取失败：${models.failures.map((f) => f.id).join("、")}`) : null) : null) : e(react.default.Fragment, null, e("div", { className: "pdf2zh-modal-desc" }, svg(ICONS.info, 14), e("span", null, "译文（.zh.md / 中英对照 .en-zh.md）的统一落盘位置与任务超时；对之后新发起的翻译生效。")), e("div", { className: "pdf2zh-field-card" }, e("div", { className: "pdf2zh-field-label" }, e("span", { className: "pdf2zh-field-icon" }, svg(ICONS.folder, 14)), "保存路径"), e("input", {
				className: "pdf2zh-input pdf2zh-input-mono",
				style: INPUT_STYLE,
				value: outputDir,
			...
			}, d.split("/").pop()))) : null, e("div", { className: "pdf2zh-field-hint" }, "需为服务器上的绝对路径（保存时自动创建）。若模型把译文写到源 PDF 旁，任务完成时插件会兜底复制到这里。")), e("div", { className: "pdf2zh-field-card" }, e("div", { className: "pdf2zh-field-label" }, e("span", { className: "pdf2zh-field-icon" }, svg(ICONS.clock, 14)), "任务超时"), e("div", { className: "pdf2zh-preset-row" }, TIMEOUT_PRESETS.map((p) => e("button", {
				key: p.v,
				type: "button",
			...
			}), e("span", { className: "pdf2zh-preset-unit" }, "分钟"))), e("div", { className: "pdf2zh-field-hint" }, "10–1440 分钟。超时后插件会终止翻译会话并把任务记为失败。"))), error !== "" ? e(Strip, {
				kind: "error",
				text: error
			}) : null, notice !== "" ? e(Strip, {
				kind: "notice",
		...
		function JobRow({ job, onDelete, onRetry }) {
			...
			return e("div", {
				...
				value: job.progress,
				status: job.status
			}), e("span", { className: `pdf2zh-job-pct pdf2zh-job-pct-${job.status}` }, `${pct}%`)), job.provider !== "" || job.modelNote !== void 0 ? e("div", { className: "pdf2zh-job-meta" }, job.provider !== "" ? e("span", { className: "pdf2zh-job-api" }, svg(ICONS.plug, 11), ` ${job.provider}/${job.model}`) : null, job.modelNote ? e("span", { className: "pdf2zh-job-warn" }, job.modelNote) : null) : null, job.status === "done" && (job.outputPaths?.length ?? 0) > 0 ? e("div", { className: "pdf2zh-job-meta" }, job.outputPaths.map((p) => e("span", {
				key: p,
				className: "pdf2zh-job-out"
			}, `→ ${p}`))) : null, job.status === "done" && (job.outputPaths?.length ?? 0) === 0 ? e("div", { className: "pdf2zh-job-meta" }, job.note ?? "翻译已完成，输出文件见会话汇报。") : null, job.note && job.status !== "done" ? e("div", { className: "pdf2zh-job-meta" }, e("span", { className: "pdf2zh-job-warn" }, job.note)) : null, job.status === "failed" && job.error ? e("div", { className: "pdf2zh-job-meta" }, e("span", { className: "pdf2zh-job-error" }, job.error), connDown ? e("span", { className: "pdf2zh-job-hintline" }, "模型服务不在线：先在服务器启动对应服务（vLLM 等），或在「⚙ 设置 → 模型 API」换用其它可用 API，然后点「重试」。") : null) : null);
		}
